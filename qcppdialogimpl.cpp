...
void QCPPDialogImpl::saveSettings()
{
   ...
   settings.setValue("/cutecom/Baud", m_baudCb->currentIndex());
   ...
   settings.setValue("/cutecom/Protocol", m_protoPb->currentIndex());
   settings.setValue("/cutecom/width", width());
   settings.setValue("/cutecom/height", height());

   settings.setValue("/cutecom/LineMode", m_inputModeCb->currentIndex());
...
//         if (!sendByte(data.data()[i]))
            ...
            || (m_protoPb->currentText()=="1kXModem"))
   {
      ...
      connect(m_sz, SIGNAL(readyReadStandardError()), this, SLOT(readFromStderr()));
      connect(m_sz, SIGNAL(finished(int,QProcess::ExitStatus)), this, SLOT(sendDone()));

      // start sh with args
      m_sz->start("sh",listProcessArgs);
      if (!m_sz->waitForStarted(3000))
      {
...
{
   QByteArray ba=m_sz->readAllStandardOutput();
...
void QCPPDialogImpl::sendDone(int exitCode,
